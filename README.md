...
**Status:** `v0.2.0` tagged. Phase 0 spikes green; Waves 1–4 delivered (virtual clock, control flow, modifier-clicks, magnify, pasteboard, async handlers, fixtures, parallel run-suite, JUnit/TAP/markdown exporters, file panels). Spec grammar `pry_spec_version: 1` frozen. Runner exposes 25+ MCP tools and a public Swift library (`PryRunner`). 9 DemoApp specs PASS, **40 unit tests** pass. Latest wave (Canopy field feedback): `type_chars` for SwiftUI `.onKeyPress` fields, `wait_for_focus`, `dump_focus`, `assert_stable` (anti-flicker), `not_equals` / `not_matches` state expectations, `expect_total:` companion to `nth:`, `sheet:` predicate, multi-line `with_fs` frontmatter, `auto_build: true` in `.pry/config.yaml`, `View.pryRegister(_:)` SwiftUI sugar, `pry_menu_inspect` / `pry_focus` MCP tools, `tree --compact`. Earlier additions: `pry-mcp lint` / `init` / `report`, `assert_focus` / `assert_eventually` / `soft_assert_state`, `select_range` / `multi_select` / `with_retry` / `copy_to`, embedded screenshots, AX tree diff, state-delta timeline, slow-step warnings, CI release pipeline + Homebrew tap auto-bump. See [`docs/ROADMAP.md`](docs/ROADMAP.md) for what's next.
